...
sim_comms <- map(unique(proc_sessions$plot_session), 
                 sample_community, reps = 1000, .progress = TRUE)

## and bind the list of data frames into one data frame
sim_comms_df <- bind_rows(sim_comms)
...
sim_comms_data <- left_join(sim_comms_df, proc_session_data, by = c("session_id", "taxon_id")) %>%
...
pele_comms <- sim_comms_data %>%
  ...
  group_by(session_id, site_id, precise_richness, proc_richness) %>%
...
t.test(pele_comms$proc_richness ~ pele_comms$over)
