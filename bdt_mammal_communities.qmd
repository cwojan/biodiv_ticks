...
session_years <- mammal_trap_df %>%
  select(plot_session, year, mean_month, mean_yday) %>%
  distinct()
...
species_by_session <- mna_by_session %>%
  ...
  select(site_id, plot_id, nlcd_class, plot_session, session, year, mean_month, mean_yday, taxon_id, presence, precise_richness)
...
proc_session_data <- proc_sessions %>%
  select(plot_session, plot_id, site_id, nlcd_class, year, mean_month, mean_yday, taxon_id, mna,
...
## write to csv
write_csv(community_effects, str_c("processed_data/mammal_community_effects_", curr_date, ".csv"))
