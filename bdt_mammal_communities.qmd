...
sim_pres_gg <- sim_presence_deviations %>%
  ...
  summarize(sim_dev_mean = mean(sim_eff),
...
## now create a plot
ggplot(data = sp_pres_comp_gg, aes(x = factor(rank), color = site_id)) +
  geom_point(aes(y = effect_size, alpha = significant)) +
  geom_errorbar(data = sim_pres_gg, aes(ymin = lower, ymax = upper, color = site_id),
                alpha = 0.5, width = 1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_alpha_manual(values = c(0.3, 1)) +
  facet_grid(rows = vars(taxon_id), cols = vars(site_id)) +
  labs(x = "Plot", 
       y = "Deviation from Expected Proportion of \nTrapping Sessions Species is Present") +
  theme_bw() +
